...
function renderSlides() {
  ...
  // Subpath-Deployments (z.B. GitHub Pages): absolute Content-Pfade auf base umschreiben
  if (BASE !== '/') {
    const prefix = BASE.replace(/\/$/, '');
    app.querySelectorAll('img[src^="/"], iframe[src^="/"], source[src^="/"], video[src^="/"]').forEach(el => {
      const v = el.getAttribute('src');
      if (v && !v.startsWith('//') && !v.startsWith(prefix + '/')) el.setAttribute('src', prefix + v);
    });
    app.querySelectorAll('a[href^="/"]').forEach(el => {
      const v = el.getAttribute('href');
      if (v && !v.startsWith('//') && !v.startsWith(prefix + '/')) el.setAttribute('href', prefix + v);
    });
  }
}
